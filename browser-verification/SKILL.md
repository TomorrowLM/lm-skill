...
4a. **登录重定向必须停止当前页面验证** — 业务页导航后若 URL path 为 `/login`，或页面明确显示登录页，立即提示用户“业务页已重定向到 `/login`，已跳过当前页面验证”，将当前范围标记为 `blocked`/`auth-required`，不采集登录表单、不输入凭据、不反复重试，也不能把登录页证据当作业务页验证结果。
...
11. **MCP 是唯一浏览器执行层** — 连接、选页、导航、滚动、查找、点击、Tab 切换、弹层检测和证据采集均通过最新 `front-automation-mcp`；禁止用截图猜结果、直接 Playwright/CUA 补采或把旧 MCP 当默认路径。MCP 返回成功后必须再次检查 URL/激活态/主要内容/截图，闭环失败就标记 `unverified`。
...
- **交互测试**：在采集结果基础上执行低风险 Tab、查看详情和可关闭弹层；每次操作都要复核 route、激活态、主内容、截图和 DOM。高风险动作除非用户明确授权，否则只验证“存在性”和权限候选，不实际执行。
