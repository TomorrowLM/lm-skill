...
执行 Phase 5 前，只读取主窗口已经生成并确认的功能技术文档 `# 10 测试用例与验收清单`。

- `# 10` 是本次自动化验证和 UI / 交互验收的唯一测试输入。
...
```mermaid
flowchart TD
	A[读取主窗口生成的功能文档 #10 测试用例与验收清单] --> B{测试和验收标准是否完整}
	B -->|否| C[返回主窗口补充 #10]
	C --> A
	...
	G --> H[执行自动化验证]
	G --> I{选择浏览器验收方式}
	I -->|用户自行手动| I1[用户按 #10 UI / 交互验收执行并反馈]
...
根据项目 `package.json`、本次改动范围和功能技术文档 `# 10` 中的自动化测试项确定需要执行的自动化验证命令。
...
浏览器验收覆盖功能技术文档 `# 10` 中适用的 UI / 交互验收和 E2E 测试项。开始前根据环境和用户意愿选择一种方式，并在 evidence 文件中记录验收方式。环境可被 AI 访问时优先使用方式 B；需要业务人员确认或 AI 无法访问环境时使用方式 A。

#### 方式 A：用户自行手动验收

用户按照功能技术文档 `# 10 UI / 交互验收` 和 `# 10 E2E 测试` 执行验收，并反馈：

- 通过或不通过。
- UI / 交互验收项和 E2E 测试项的实际结果。
- 问题截图和复现步骤。
- 设备、浏览器和屏幕尺寸。
- 未验证项和阻塞原因。
...
由 AI 在浏览器中执行页面操作、视觉检查、网络检查、控制台检查和 DOM 检查，并覆盖 `# 10 E2E 测试` 中可在浏览器完成的完整业务流程、接口、权限和异常场景；用户负责提供必要的访问权限和环境信息。推荐由独立验收子窗口执行，主窗口应：

1. 将 `# 10 UI / 交互验收` 的入口、验收项和环境要求挂载到验收任务，并依据 `# 10` 中的单元测试、组件测试和 E2E 测试项确认自动化验证范围。
...
|------|----------|------|--------|----------|----------|
| `...` | `# 10` 中的自动化测试要求 | 通过 / 失败 / 未执行 / 不适用 | ... | ... | ... |

## 手动验收

| 验收项 | 结果 | 操作或观察证据 | 截图 / 复现步骤 |
|--------|------|----------------|----------------|
| `# 10 UI / 交互验收` 或 `# 10 E2E 测试` 中的验收项 | 通过 / 失败 / 未验证 / 不适用 | ... | ... |
