...
5. 页面或完整功能模块默认必须在 Phase 3 生成 `spec/implementation-plan.md` 或拆分后的 `spec/*-spec.md`；只有用户明确表示“不需要 spec”或“不落盘 spec”时，才允许记录豁免并使用内联任务。
...
- **接口契约来源**：页面涉及接口时，默认先读取 `mcp-exe` 并调用 `lm-mcp-server.get_swagger_mcp`；只有查询失败或结果不足以形成可用契约时，才加载 `apifox-cli` 作为回退。两者结果冲突时不得静默覆盖，必须记录冲突并暂停确认。
- **任务规格与依赖**：默认任务必须绑定对应 spec；依赖任务只有在前置任务完成并由主 Agent 审查为 `reviewed` 后才能打开。用户明确豁免 spec 时，必须在 Phase 3 记录原始豁免理由、任务边界和验收标准。
- **接口来源记录**：方案和 spec 必须逐接口记录实际来源工具、查询 source、获取结果；Apifox 回退时记录 Swagger 失败原因、项目/分支/资源标识，不得把 Apifox 结果标记为已通过 Swagger。
