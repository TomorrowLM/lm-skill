...
**官方指南：** Anthropic 官方的技能编写最佳实践请参见 `references/guides/anthropic-best-practices.md`。该文档提供了补充本技能 TDD 导向方法的额外模式和指南。
...
2. **可复用工具** - 脚本、实用程序、模板
3. **主 SKILL.md 将超过 300 行** - 把大段参考内容拆到 `references/` 中，并在主文件中引用
...
**硬限制：** 单个 `SKILL.md` 不得超过 300 行。

如果预计会超过 300 行，不要继续把内容堆在主文件里，而要立刻拆分：

1. 把高密度参考内容、长示例、扩展说明下沉到当前 skill 目录下的 `references/`
2. 主 `SKILL.md` 只保留触发条件、核心原则、关键流程、红线和对 `references/` 模块的引用
3. `references/` 本身只放子文件夹；实际文件按主题放入这些子文件夹中，例如 `references/examples/usage.md`、`references/guides/review-checklist.md`
4. 在主 `SKILL.md` 中明确写出要读取哪个 `references/` 文件，而不是只说“见参考资料”

**拆分原则：** 主文件负责路由和决策，`references/` 负责展开细节；不要把影响执行顺序的硬规则藏进 `references/`。
...
**判断规则：**
1. 如果模型不读这段内容就可能走错流程，这段内容必须留在主 `SKILL.md`
2. 如果这段内容只是在解释“为什么这样做”或“具体可以怎么展开”，默认应进入 `references/`
3. 不要等写到 300 行才拆；只要已经出现“控制逻辑”和“展开说明”混写，就应主动分层
4. 主文件中必须明确写出“什么时候读哪个 `references/` 文件”，不能只留下目录或笼统的“详见参考资料”

**目录规则：** `references/` 目录本身只放子文件夹，不直接放 `.md`、`.dot`、模板或示例文件；示例、说明、模板、扩展材料都应落在对应的子目录中。
...
如果这份结构写完后会超过 300 行，优先把长示例、补充表格、术语表和扩展案例拆到 `references/`，不要把主文件继续写成巨型手册。

一个实用标准是：主文件回答“什么时候进入、进入后必须怎么走”；`references/` 回答“这一段怎么展开、有哪些例子、有哪些补充说明”。
...
参见 `references/conventions/graphviz-conventions.dot` 了解 graphviz 样式规则。
...
**心理学说明：** 理解说服技巧为什么有效有助于你系统性地应用它们。参见 `references/guides/persuasion-principles.md` 了解研究基础（Cialdini, 2021; Meincke et al., 2025），涵盖权威、承诺、稀缺、社会认同和归属原则。
...
**测试方法论：** 参见 `references/methodology/testing-skills-with-subagents.md` 了解完整的测试方法：
- 如何编写压力场景
- 压力类型（时间、沉没成本、权威、疲惫）
- 系统地堵住漏洞
- 元测试技巧
