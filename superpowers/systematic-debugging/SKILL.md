...
### GitNexus 增强（条件性，有索引时优先执行）

当项目已有 GitNexus 索引时，在第一阶段之前先执行 `gitnexus-debugging` 技能的工作流（query → context → process → cypher），快速构建 bug 周围的代码地图。
...
5. **跟踪数据流**
   ...
   **当错误发生在调用栈深处时：**
   ...
   参见 `references/auxiliary-techniques/root-cause-tracing.md`，了解完整的反向追踪技术。
...
**修复前必须知道爆炸半径。** 当项目已有 GitNexus 索引时：
...
没有 GitNexus 索引时，手动评估修改点的调用方和依赖方。
...
3. **验证修复**
   ...
   - 其他测试没有被破坏吧？
   - 问题真的解决了吗？
   - 使用 `superpowers:verification-before-completion` 技能的完整门控方法论
   - 有 GitNexus 索引时，增加 `gitnexus_detect_changes({scope: "staged"})` 确认变更范围与预期一致
...
**当生产环境全面中断（服务不可用、数据持续损坏）时：**

允许「止血 + 根因调查」并行，但必须满足全部条件：

1. **止血措施是可逆的** — 加 retry 必须配超时上限和熔断，不能留一个永久绕过
2. **止血不等于修完** — 部署止血后立即进入 Phase 1，不允许"之后再说"
3. **记录决策** — 写下"我部署了 X 作为止血，根因调查待完成"
4. **设置跟踪** — 如果 24 小时内没有完成根因调查，止血措施必须自动告警

**不是所有紧急情况都是"全面中断"。** 部分降级、单个用户受影响、有 workaround 的场景 → 不适用此例外，走正常流程。
...
当你是唯一坚持流程的人（资深工程师说"这样就行"、领导要求立刻修好、全组想结束讨论）：

1. **用时间盒限定调查** — "给我 10 分钟追踪数据流，如果没有发现再按当前方案走"
2. **用提问代替对抗** — 不问"为什么不等"，而问"middleware 的 token 生命周期是什么？"（把流程转化为技术问题）
3. **记录你的顾虑** — "我担心 X，建议至少跑一下 Y 验证"（留下书面记录比口头坚持更有效）
4. **接受决策但保持警觉** — 如果团队最终选择了快速方案，主动跟进：修复生效了吗？需要后续排查吗？

**核心：不是对抗权威，而是把"要不要走流程"转化为"花 N 分钟验证一个具体问题"。**
...
| "紧急情况，没时间走流程" | 系统化调试比反复猜测式修复更快。全面中断时见「紧急止血例外」。 |
...
- **`references/auxiliary-techniques/root-cause-tracing.md`** - 沿调用栈反向追踪 bug，找到最初的触发点
- **`references/auxiliary-techniques/defense-in-depth.md`** - 找到根因后，在多个层级添加校验
...
完整的 GitNexus 工具参数和示例见本目录下的 `references/gitnexus-toolkit.md`。
