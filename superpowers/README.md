...
| code-rule | AI 将要编写前端代码、建立前端规范或审查前端规范合规性时 | using-superpowers / writing-plans / executing-plans | verification-before-completion / chinese-code-review | 在实现前先加载前端编码规范，约束 AI 的代码输出。 |
...
using-superpowers → brainstorming → writing-plans → code-rule（如涉及前端代码）→ subagent-driven-development 或 executing-plans → verification-before-completion → requesting-code-review → receiving-code-review → finishing-a-development-branch
