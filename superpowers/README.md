 ...
 **子智能体（Subagent）** 并不是指另一个打开的聊天窗口或同一个会话的延续，而是一个**全新的、上下文隔离的、一次性的执行者**。

它有几个关键点：

1. **独立性**：它是一个独立的执行单元，**不继承**主会话（Main Session）的历史聊天记录、之前的文件浏览记录或已形成的临时判断。
   1. 上下文独立
   2. 任务边界独立
2. **一次性**：每个任务通常对应一个全新的子智能体。任务 A 用完即止，任务 B会启动一个新的子智能体。
3. **受控性**: 它由主会话（作为控制者/项目经理）调度，只负责执行被分派的特定小块任务。
