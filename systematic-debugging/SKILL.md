...
在满足下面 5 个门禁前，不要提出修复方案、不要改代码、不要加 workaround：

1. **复现门禁**：能说明复现命令/步骤、实际结果、期望结果；不能复现时先补日志或收集证据。
2. **证据门禁**：完整阅读错误、堆栈、失败断言、相关日志，并记录关键文件、行号、错误码或异常值。
...
5. **跨边界取证**：多组件系统要在组件边界记录输入、输出、配置和状态，只打一轮诊断补丁来定位断裂点。
...
1. 先创建失败测试、最小复现脚本，或明确可重复验证命令；能用 `test-driven-development` 时使用它。
2. 只修已确认根因，不顺手重构，不捆绑无关优化。
3. 运行复现验证、相关测试和必要的 lint/typecheck/build。
4. 完成前使用 `verification-before-completion` 的门控：只根据实际命令输出声明成功。
5. 有 GitNexus 索引时，修复后运行变更检测，确认影响范围符合预期。

如果修复失败：

- 第 1 次或第 2 次失败：停止叠补丁，回到第 1 阶段补证据。
- 第 3 次失败：默认存在架构或模型假设错误，暂停继续修，重新讨论方案。
...
1. 请求一个短时间盒：“给我 10 分钟验证 token 在 middleware 前后是否变化。”
2. 用具体问题替代反对：“这个状态在哪一层从 completed 变成 pending？”
3. 记录风险：“我担心在报错点兜底会掩盖上游字段丢失，建议至少跑 X 验证。”
4. 如果团队决定先合入，继续跟踪验证结果和根因补查。
