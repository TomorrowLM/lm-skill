...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **必须假设以下内容你都不知道，除非它们已经明确写在本提示词里：**
    - 主会话之前讨论过什么
    - 主会话刚才看过哪些文件
    - 主会话已经尝试过哪些方案
    - “同类表单/同类页面/刚才那个字段”采用了什么做法
    ...
    **不要说或暗示：**
    ...
    如果当前提示词没有明确提供这些信息，你只能：
    1. 基于这里明确给出的任务文本和上下文工作
    2. 在缺少关键信息时立即提问
    3. 明确说“当前分派上下文没有提供这部分信息”
    ...
    **现在就问。** 在开始工作之前提出任何疑虑。
    ...
    当你确认需求清晰后：
    1. 严格按照任务指定的内容实现
    2. 如果任务涉及行为变更、Bug 修复或重构，衔接 TDD；纯配置、文档和机械修改不强制套用 TDD
    3. 验证实现是否正常工作
    4. 按项目流程决定是否提交；不要自行创建、合并或改写共享分支历史
    5. 自审（见下文）
    6. 汇报
    ...
    **工作过程中：** 如果遇到意料之外或不清楚的情况，**提问**。
    ...
    ## GitNexus 代码探索（有索引的项目）

    在开始编码前，如果项目有 GitNexus 索引，用它快速了解现有代码结构，
    而不是逐个读文件盲目摸索。

    **检查索引是否可用：**
    1. READ gitnexus://repo/{name}/context
       - 如果返回正常数据 → 索引可用，继续下面的步骤
       - 如果返回 "No index found" 或 "Index is stale" → 跳过本节，不要用 CLI 重建索引

    **探索现有代码（按需使用，不要对每个任务都跑一遍）：**
    - gitnexus_query({query: "<任务涉及的业务概念>"})
      → 找到与当前任务相关的现有模块和执行流
    - gitnexus_context({name: "<要复用或依赖的符号>"})
      → 了解现有组件的调用方、被调用方和所属流程

    **什么时候值得探索：**
    - 任务需要调用或依赖现有的模块/组件/服务
    - 需要了解现有的模式（状态管理、API 组织、错误处理等）
    - 任务描述提到了你不熟悉的现有代码

    **什么时候跳过：**
    - 任务是纯新建（类型定义、常量配置、独立组件等）
    - 上下文中已经提供了足够的代码结构信息
    - 索引不可用
