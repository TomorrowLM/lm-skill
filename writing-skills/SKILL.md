...
- 不影响执行顺序的内容下沉到 `references`；250 行只是维护建议，不是硬门槛。超过后检查重复、长示例、上下文负担和简单任务额外开销，再决定是否拆分。
...
- 每次创建或修改技能时，必须新增或更新该技能的 `references/tests/` 测试用例文件；不能只在对话中临时编写用例。
- 默认使用 `references/tests/evals.json`，也可以按用途拆成多个 JSON 文件；文件至少覆盖适用的触发、行为和压力场景，并为每条用例提供稳定的 `id`、输入和期望行为。
- 测试结果必须引用用例 `id`；临时提示词、代理口头报告和旧测试日志不能替代可复用的测试用例文件。
...
| Token 成本 | 简单任务是否引入不成比例开销，控制逻辑与 references 是否合理分层；行数仅作维护参考。 |

### Token 成本评估（简化版）

目标：确认技能不会为简单任务引入不成比例的开销。**不要求子代理自报估算数（不可靠），只检查结构负担和简单任务开销：**

1. **结构负担**：检查是否有重复规则、长示例、背景说明或可按需读取的细节；250 行只是提醒复查的信号，不能单独判定失败。只有拆分能降低上下文负担或改善渐进式披露时才拆分。
2. **简单任务开销**：挑一条最简单的触发用例，肉眼对比带技能与基线的隔离智能体输出。如果带技能输出明显更长（感官上 > 50%），且多出的内容只是技能自身的流程说明模板而非解决任务所需，则把模板内容下沉到 `references/`。

两点都通过，结论写"可接受"。行数超过 250 但没有造成重复或不成比例开销时，可以保留；不需要全量采集，不需要字符→token 换算。

### 测试记录格式

每条用例至少记录：测试文件路径、用例 ID、用例文本、是否应触发、实际是否触发、是否遵守关键规则、失败原因、下一步修改点。
