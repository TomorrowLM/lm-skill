...
1. [概述](#概述)
2. [何时使用](#何时使用)
3. [TDD 映射到技能测试](#tdd-映射到技能测试)
4. [红色阶段：基线测试（观察失败）](#红色阶段基线测试观察失败)
5. [绿色阶段：编写最小技能（使其通过）](#绿色阶段编写最小技能使其通过)
6. [验证绿：压力测试](#验证绿压力测试)
7. [重构阶段：堵住漏洞（保持绿色）](#重构阶段堵住漏洞保持绿色)
8. [元测试（当绿色不起作用时）](#元测试当绿色不起作用时)
9. [技能何时无懈可击](#技能何时无懈可击)
10. [示例：TDD 技能的加固过程](#示例tdd-技能的加固过程)
...
**必需背景：** 在使用此技能前，你必须理解红-绿-重构循环。本文件只提供技能专用的测试格式：压力场景、合理化借口和复测方式。
