...
- 工具输出可能超过 200 行。
- 任务跨越多个阶段、多个窗口或多个子代理。
- 任务已拆成多个相对独立的子任务；此时优先接入 `subagent-driven-development`，本技能只负责压缩上下文和控制 token。
...
2. 如果任务已拆成独立子任务，优先 `subagent-driven-development`。
...
大量文件读取场景优先用子代理（`runSubagent`）承载；如果任务已经拆成多个相对独立的子任务，直接接入 `subagent-driven-development`，按任务边界分派和两阶段审查执行。
...
- 当任务已经拆成独立子任务时，应优先接入 `subagent-driven-development`，不要用伪装式的 fork 方案替代真实子代理隔离。
