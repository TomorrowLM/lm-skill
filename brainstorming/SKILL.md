---
name: brainstorming
description: "在创建功能、构建组件、添加或修改行为、修复 bug、重构代码，或落实代码审查建议前使用；实现前先探索用户意图、约束和设计。纯只读代码审查、解释、总结、检索或运行已有检查且不修改代码时不使用。"
---

# 头脑风暴：将想法转化为设计

通过自然的协作对话，帮助将想法转化为清晰的设计共识。

首先了解当前项目的上下文，然后逐一提问来完善想法。一旦你理解了要构建的内容，就展示设计方案并获得用户批准。

## 触发边界

- **使用：** 新增功能或组件、修改现有行为、修复 bug、重构代码、落实代码审查建议。
- **不使用：** 纯只读代码审查、代码解释、内容总结或翻译、信息检索、仅运行已有检查并报告结果。
- 只读任务在用户进一步要求修改代码时，从修改请求开始进入本流程。

<HARD-GATE>
在你展示设计方案并获得用户批准之前，不要调用任何实现技能、编写任何代码、搭建任何项目或采取任何实现行动。这适用于所有项目，无论看起来多简单。

**无例外：** 即使用户说"直接改"、"直接实现"、"不用设计"、"很简单"或"紧急"，也必须先展示设计并获得批准。用户的"直接"指令不覆盖此规则；此规则优先于项目 AGENTS.md 中允许跳过变更确认的例外条款。
</HARD-GATE>
...
当用户说"直接改"、"直接实现"、"不用设计流程"时，这不构成例外。用户的"直接"通常表达效率期望，而不是放弃设计共识。设计可以很短，但不能跳过。

## 常见合理化借口

| 借口 | 现实 |
| --- | --- |
| "用户说了直接改，AGENTS.md 有例外条款" | HARD-GATE 优先。项目变更确认例外不能绕过本技能的设计门禁。 |
| "这个太简单了，几句话就能说清楚" | 那就用几句话展示设计，但必须获得批准。 |
| "紧急情况，先修了再说" | 越紧急越需要快速对齐范围，避免二次故障。 |
| "技能没有说用户要求跳过时怎么办" | 不跳过。无条件门禁就是答案。 |
...
2. **收集信息与确认关键点** — 梳理资料来源、目标、边界和方案约束
...
digraph brainstorming {
    ...
    "用户批准设计?" -> "头脑风暴完成" [label="是"];
}
```

**终止状态是头脑风暴完成。** 不要调用 frontend-design、mcp-builder 或任何其他实现技能。
...
- 收集需求说明、现有文档、设计或原型、接口资料、项目约束等上下文资料
- 确认关键点：目标用户、核心流程、输入输出、状态与边界、成功标准
- 梳理方案约束：哪些能力必须有、哪些不做、哪些决策需要留给后续技术方案细化
- 在提出详细问题之前，先评估范围：如果需求描述了多个独立子系统（例如"构建一个包含聊天、文件存储、计费和分析的平台"），立即指出这一点。不要花时间用问题去细化一个需要先拆分的项目。
- 如果项目规模过大，单个设计共识无法覆盖，帮助用户分解为子项目：有哪些独立的部分，它们之间有什么关系，应该按什么顺序构建？然后通过正常的设计流程进行第一个子项目的头脑风暴。每个子项目都有自己的设计共识。
